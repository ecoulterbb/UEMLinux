...
Usage:
  ...
  python3 uem_install.py --yes        # non-interactive (defaults for all prompts)
...
AUTO_YES = False
KEYSTORE_JKS_PASSWORD = "aod8T2mx9KuA"
SCRIPT_DIR = Path(__file__).resolve().parent
PROFILE_DIR = SCRIPT_DIR / "profiles"
MACHINE_PROFILE = PROFILE_DIR / "core-ui-linux-onprem.machine.properties"
PARTITION_GCS_PROFILE = PROFILE_DIR / "linux-onprem-partition-gcs.properties"
DB_FIXES_PROFILE = PROFILE_DIR / "core-ui-linux-onprem-db-fixes.properties"


def _profile_substitutions(cfg, hostname, core_ui, log_root):
    """Build placeholder map for profile .properties files."""
...
def _substitute_profile_text(text, subs):
    for key, value in subs.items():
        text = text.replace(f"%{{{key}}}", str(value))
    return text


def _merge_profile_keys_into_text(text, profile_path, subs):
    """Overlay keys from a profile file onto existing properties text."""
    if not profile_path.exists():
        raise FileNotFoundError(f"Profile not found: {profile_path}")
    overlay = _substitute_profile_text(profile_path.read_text(), subs)
    merged = dict(_parse_properties_text(text))
    for key, value in _parse_properties_text(overlay).items():
        merged[key] = value
    return _format_properties_text(merged)
...
def _format_properties_text(props):
    return "\n".join(f"{k}={v}" for k, v in props.items()) + "\n"


def build_machine_properties(cfg, hostname, core_ui, log_root):
    """Render machine.properties from the Core+UI Linux on-prem profile."""
    subs = _profile_substitutions(cfg, hostname, core_ui, log_root)
    if not MACHINE_PROFILE.exists():
        raise FileNotFoundError(f"Machine profile not found: {MACHINE_PROFILE}")
    body = _substitute_profile_text(MACHINE_PROFILE.read_text(), subs)
    return f"# Generated by uem_install.py — {datetime.utcnow().isoformat()}\n{body}"


def _merge_partition_gcs_profile(partition_text, cfg, hostname):
    """Append/override partition.properties with linux-onprem-partition-gcs profile."""
    core_ui = Path(cfg.get("install_root", "/opt/blackberry/uem")) / "CoreUILinux"
    log_root = cfg.get("log_root", str(core_ui / "logs"))
    subs = _profile_substitutions(cfg, hostname, core_ui, log_root)
    return _merge_profile_keys_into_text(partition_text, PARTITION_GCS_PROFILE, subs)


def _locate_gcs_dataloader_files(db_linux):
    """Return (GlobalConfigurationSetting.xml, ConfigurationSettingDefinition.xml)."""
    prod = db_linux / "mdm.dal" / "data" / "platform" / "prod"
    gcs_xml = prod / "GlobalConfigurationSetting.xml"
    csd_xml = prod / "ConfigurationSettingDefinition.xml"
    if gcs_xml.exists() and csd_xml.exists():
        return gcs_xml, csd_xml

    tools_lib = db_linux / "tools" / "lib"
    meta_jars = sorted(tools_lib.glob("mdm.core.metadata*.jar"))
    if not meta_jars:
        return None, None

    prod.mkdir(parents=True, exist_ok=True)
    for name in ("GlobalConfigurationSetting.xml", "ConfigurationSettingDefinition.xml"):
        dst = prod / name
        if dst.exists():
            continue
        r = subprocess.run(
            ["jar", "xf", str(meta_jars[-1]), f"data/platform/prod/{name}"],
            cwd=str(db_linux / "mdm.dal"),
            capture_output=True, text=True,
        )
        if r.returncode != 0:
            _dbg(f"jar extract {name} failed: {r.stderr.strip()}")
    if gcs_xml.exists() and csd_xml.exists():
        return gcs_xml, csd_xml
    return None, None


def _run_gcs_builder(db_linux, partition_props):
    """Patch GlobalConfigurationSetting.xml from partition.properties gcs.* keys."""
    gcs_xml, csd_xml = _locate_gcs_dataloader_files(db_linux)
    if not gcs_xml or not csd_xml:
        warn("GlobalConfigurationSetting.xml not found — skipping GCS builder")
        return False

    tools_lib = db_linux / "tools" / "lib"
    builder_jars = sorted(tools_lib.glob("mdm.deployment.tools*.jar"))
    if not builder_jars:
        warn("mdm.deployment.tools JAR not found — skipping GCS builder")
        return False

    classpath = f"{db_linux}:{tools_lib}/*"
    cmd = (
        f'java -cp "{classpath}" '
        f"com.rim.mdm.deployment.tools.jaxb.GlobalConfigurationSettingsBuilder "
        f'"{partition_props}" "{gcs_xml}" "{csd_xml}"'
    )
    info(f"Running GlobalConfigurationSettingsBuilder on {gcs_xml.name}")
    r = subprocess.run(cmd, shell=True, cwd=str(db_linux), capture_output=True, text=True)
    if r.returncode != 0:
        warn(f"GlobalConfigurationSettingsBuilder exited {r.returncode}")
        if r.stderr:
            _dbg(r.stderr.strip()[:2000])
        if r.stdout:
            _dbg(r.stdout.strip()[:2000])
        return False
    ok(f"Patched {gcs_xml.relative_to(db_linux)} from partition GCS profile")
    return True


def _load_db_fixes_from_profile(cfg):
    """Load (update_name, value) and (insert_name, value) tuples from db-fixes profile."""
    if not DB_FIXES_PROFILE.exists():
        return [], []
    hostname = cfg.get("hostname", socket.gethostname())
    core_ui = Path(cfg.get("install_root", "/opt/blackberry/uem")) / "CoreUILinux"
    log_root = cfg.get("log_root", str(core_ui / "logs"))
    subs = _profile_substitutions(cfg, hostname, core_ui, log_root)
    text = _substitute_profile_text(DB_FIXES_PROFILE.read_text(), subs)
    updates, inserts = [], []
    for line in text.splitlines():
        stripped = line.strip()
        ...
        if key.startswith("insert."):
            inserts.append((key[len("insert."):], value))
        else:
...
def phase_tarball(state, cfg):
    ...
    if found:
        if AUTO_YES:
            ...
            ok(f"Auto-selected tarball: {tarball_path}")
        else:
    ...
    if not tarball_path or not tarball_path.exists():
        auto_tar = Path("/home/uem/uem_install_pkg/uem.catalog.cloud-43.32.0.tar")
        if AUTO_YES and auto_tar.exists():
            tarball_path = auto_tar
        else:
...
def phase_config(state, cfg):
    ...
    required = {
        ...
        "ssl.db.key.store":       "",
        "ssl.db.key.password":    "",
        # installKeystore / installCertificates — dot + underscore keys (on-prem)
        "array.keystore.types":           "CACERTS,ATTESTATION,APPLE",
        "cacerts.keystore.location":      "keystore.jks",
        "cacerts.keystore.password":      KEYSTORE_JKS_PASSWORD,
        ...
        "certificate.store.location":     "keystore.jks",
    }
    ...
    try:
        text = _merge_partition_gcs_profile(text, cfg, hostname)
        ok("partition.properties — merged linux-onprem-partition-gcs profile")
    ...
    try:
        machine_content = build_machine_properties(cfg, hostname, core_ui, log_root)
    except FileNotFoundError as ex:
        err(str(ex))
        return False

    machine_props.write_text(machine_content)
    ok("machine.properties created (profiles/core-ui-linux-onprem.machine.properties)")
...
def _extract_deployment_jks(db_linux: Path) -> bool:
    """Extract bundled JKS files from mdm.dal.deployment-*.jar into DatabaseLinux/."""
    tools_lib = db_linux / "tools" / "lib"
    jars = sorted(tools_lib.glob("mdm.dal.deployment-*.jar"))
    if not jars:
        warn(f"mdm.dal.deployment JAR not found under {tools_lib}")
        return False
    jar = jars[-1]
    names = ["keystore.jks", "keystore_prod.jks", "apple_prod.jks", "attestation_prod.jks"]
    ok_count = 0
    for name in names:
        dst = db_linux / name
        if dst.exists():
            ok_count += 1
            continue
        r = subprocess.run(["jar", "xf", str(jar), name], cwd=str(db_linux),
                             capture_output=True, text=True)
        if r.returncode == 0 and dst.exists():
            ok_count += 1
            _dbg(f"Extracted {name} from {jar.name}")
        else:
            warn(f"Could not extract {name} from {jar.name}: {r.stderr.strip()}")
...
def phase_db_deploy(state, cfg):
    ...
    section("Extract deployment keystores")
    if _extract_deployment_jks(db_linux):
        ok("keystore.jks + prod JKS files ready in DatabaseLinux/")
    else:
        warn("Some JKS files missing — installKeystore/installCertificates may fail")

    section("Patch GlobalConfigurationSetting.xml from partition GCS profile")
    partition_props = db_linux / "context" / "partition.properties"
    if partition_props.exists():
        _run_gcs_builder(db_linux, partition_props)
    else:
        warn(f"{partition_props} missing — skipping GCS builder")
    ...
    section("Step 2 — installKeystore, installCertificates, DNA, version metadata")
    info("Runs: installKeystore → installCertificates → installDna → setMetadataVersion → updateDbVersion → runSqlScripts")
...
def _apply_db_fixes(cfg):
    ...
    #
    #     Non-host values below are loaded from profiles/core-ui-linux-onprem-db-fixes.properties
    _profile_fixes, _profile_inserts = _load_db_fixes_from_profile(cfg)
    if not _profile_fixes and not DB_FIXES_PROFILE.exists():
        warn(f"DB fixes profile not found: {DB_FIXES_PROFILE}")
    ...
    #     upgrade/re-contextualization safety net.
    _cirrpki_fixes = [
...
def _import_blackberry_root_ca(cfg):
    """No-op — aaa_root is loaded from keystore.jks via installCertificates()."""
    _dbg("_import_blackberry_root_ca skipped (DB trust via installCertificates)")


def _dna_compliance_def_count(cfg):
    conn = _db_connect(cfg)
    cur  = conn.cursor()
    cur.execute("""
        SELECT count(*) FROM def_policy_setting_definition
        WHERE internal_id LIKE 'COMPLIANCE.%'
    """)
    n = cur.fetchone()[0]
    cur.close()
    conn.close()
...
def _wait_for_dna_policy_definitions(cfg, timeout_s=600):
    """
    Wait for Core's DNASchemaService to promote DNA artifacts into
    def_policy_setting_definition.  Phase 6 installDna() only loads the
    binary store; tenant provisioning needs the runtime promotion step.
    """
    deadline = time.time() + timeout_s
    while time.time() < deadline:
        try:
            n = _dna_compliance_def_count(cfg)
            if n > 0:
                return n
        except Exception as e:
            _dbg(f"DNA policy def check failed: {e}")
        time.sleep(10)
    return 0
...
""")
    ...
    section("DNA policy definitions")
    info("Phase 6 installDna() loads DNA zips into o2o_binary_store_dna_artifact.")
    info("Core promotes them into def_policy_setting_definition on startup —")
    info("the first boot can time out; a restart usually completes promotion.")

    dna_count = _wait_for_dna_policy_definitions(cfg, timeout_s=300)
    if dna_count == 0:
        warn("DNA policy definitions not loaded after 5 min — restarting Core once...")
        run("pkill -9 -f 'tomcat-core'", check=False)
        time.sleep(5)
        run(f"bash {launcher}", check=False)
        if not _stream_startup_log(core_log, 8887, 600, "Core"):
            err("Core did not restart for DNA promotion retry")
            return False
        dna_count = _wait_for_dna_policy_definitions(cfg, timeout_s=600)

    if dna_count == 0:
        err("DNA policy definitions still missing — PICW tenant registration will fail")
        info("Check Core log for DNASchemaService / TransactionTimedOutException")
        if not confirm("Mark phase complete anyway?", default="n"):
            return False
    else:
        ok(f"DNA policy definitions loaded ({dna_count} COMPLIANCE entries)")
...
def main():
    ...
    parser.add_argument("--yes",    action="store_true",
                        help="Non-interactive: accept defaults and skip confirmation prompts")
    ...
    if not AUTO_YES and not sys.stdin.isatty():
        print("ERROR: This wizard must be run at an interactive terminal, or use --yes.")
        print("       Connect via SSH and run: python3 uem_install.py --yes")
        sys.exit(1)
...
""")
    ...
    if "deployment_type" not in cfg:
        if AUTO_YES:
            cfg["deployment_type"] = "core_and_ui"
            cfg["db_existing"] = False
        else:
            print(f"""
